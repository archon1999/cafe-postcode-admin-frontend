...
import { getFeatureKitchenModeTranslationKey, getFeatureOrderEntryModeTranslationKey } from '../../helpers/presenters';

import {
  DEFAULT_FEATURE_CONFIGS_GRID_FILTERS,
  type FeatureConfigsGridFilters,
  FeatureConfigsGridToolbar,
} from './FeatureConfigsGridToolbar';

export function FeatureConfigsGrid() {
  ...
  const [filters, setFilters] = useState<FeatureConfigsGridFilters>(DEFAULT_FEATURE_CONFIGS_GRID_FILTERS);
  ...
  const query = useGetFeatureConfigsListQuery({
    ...
    orderEntryModeIn: filters.orderEntryModes.length ? filters.orderEntryModes.join(',') : undefined,
    kitchenModeIn: filters.kitchenModes.length ? filters.kitchenModes.join(',') : undefined,
    ordering: getOrderingFromSortModel(sortModel),
  });

  const hasActiveFilters = Boolean(filters.search || filters.orderEntryModes.length || filters.kitchenModes.length);
  const handleFiltersChange = useCallback((next: FeatureConfigsGridFilters) => {
    setFilters(next);
