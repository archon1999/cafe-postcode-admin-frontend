  ...
  return (
    <Box
      ...
      <Form methods={methods} onSubmit={submit}>
        <DialogTitle sx={{ pb: 0.5, borderBottom: 1, borderColor: 'divider' }}>
          <Typography component="span" variant="h6">
            {group ? 'Xususiyatni tahrirlash' : 'Yangi xususiyat guruhi'}
          </Typography>
        </DialogTitle>

        <DialogContent sx={{ pt: '16px !important', flex: 1, minHeight: 0, overflowY: 'auto' }}>
          <Stack spacing={3}>
            <Box
              sx={{
                display: 'grid',
              ...
              {fields.map((field, index) => (
                <Box
                  ...
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25} alignItems={{ sm: 'center' }}>
                    ...
                    />
                    <RHFCheckbox<ModifierGroupFormInput>
                      name={`options.${index}.isDefault`}
                      label="Oldindan tanlangan"
                    />
                    <IconButton
