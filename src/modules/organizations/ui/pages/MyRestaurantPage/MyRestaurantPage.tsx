...
import { RestaurantOperationalManagementCard } from '../../components/RestaurantOperationalManagementCard';
...
const MyRestaurantPage = () => {
  ...
  const hasCashDeskManagement = Boolean(profile?.permissionCodes?.includes('cashdesk.manage'));
  const hasIntegrationsManagement = Boolean(profile?.permissionCodes?.includes('integrations.manage'));
  ...
  return (
    <Content>
      <CustomBreadcrumbs
        heading={t('pages.myRestaurant.title', { defaultValue: 'Mening restoranim' })}
        links={[{ name: t('pages.myRestaurant.title', { defaultValue: 'Mening restoranim' }) }]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1.1fr 1fr' }, gap: 3 }}>
        <Card sx={{ p: 3 }}>
          <Stack spacing={3}>
            ...
            <Box
              sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, minmax(0, 1fr))' }, gap: 2.5 }}>
              <Stack spacing={0.5}>
      ...
      <RestaurantOperationalManagementCard
        showCashDeskManagement={hasCashDeskManagement}
        showIntegrationsManagement={hasIntegrationsManagement}
      />
    </Content>
  );
};
