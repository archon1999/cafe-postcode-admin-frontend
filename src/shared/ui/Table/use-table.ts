...
export type RowsPerPage = 1 | 10 | 20 | 50 | 100;
...
export function useTable(props?: UseTableProps): UseTableReturn {
  ...
  const [rowsPerPage, setRowsPerPage] = useState<RowsPerPage>(props?.defaultRowsPerPage ?? 10);
  ...
  const onChangeRowsPerPage = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    setPage(1);
    setRowsPerPage(parseInt(event.target.value, 10) as RowsPerPage);
  }, []);
