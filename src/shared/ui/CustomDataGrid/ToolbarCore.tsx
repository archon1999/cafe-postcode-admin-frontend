...
import { ExportIcon, FilterIcon, ViewColumnsIcon } from 'app/theme/core/components/mui-x-data-grid';
...
export function ToolbarButtonBase({ sx, label, icon, showLabel = true, ...other }: ToolbarButtonBaseProps) {
...
export function CustomToolbarColumnsButton({ showLabel }: Pick<ToolbarButtonBaseProps, 'showLabel'>) {
  ...
  return (
    <ColumnsPanelTrigger
      render={(props) => (
        <ToolbarButtonBase {...props} label={String(label)} icon={<ViewColumnsIcon />} showLabel={showLabel} />
      )}
    />
  );
}

export function CustomToolbarFilterButton({ showLabel }: Pick<ToolbarButtonBaseProps, 'showLabel'>) {
  const apiRef = useGridApiContext();
...
export function CustomToolbarExportButton({ showLabel }: Pick<ToolbarButtonBaseProps, 'showLabel'>) {
  const apiRef = useGridApiContext();
...
export function CustomToolbarQuickFilter({ sx, slotProps, ...other }: CustomToolbarQuickFilterProps) {
  const apiRef = useGridApiContext();
  ...
  return (
    <QuickFilter
      {...other}
      render={(props) => (
        <Box {...props} sx={[{ width: 1, maxWidth: { md: 260 } }, ...(Array.isArray(sx) ? sx : [sx])]}>
          <QuickFilterControl
